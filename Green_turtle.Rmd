...
Overall, female turtles are larger than male turtles 
```{r}
green_turtle %>%
  group_by(sex) %>%
  filter(straightline_carapace_length != "NA") %>% 
...
there is an exponential growth between age 5 to 12, followed by decrease in growth rate after the age of 12
```{r}
green_turtle %>% 
  group_by(age) %>% 
...
# At what age do turtles get stranded the most?
female = 8
male = 7
```{r}
green_turtle %>% 
...
# Is there a year with the most amount of strandings?
...
green_turtle %>% 
  ...
  labs(title = "Top 5 years of green turtle stranding",
       ...
       y = "Number of stranding")+
  theme(plot.title=element_text(size=rel(1.75), hjust = 0.5))+
  scale_x_discrete() # scale the year to discrete
```
...
```{r}
green_turtle %>% 
  filter(sex == "Male") %>% # filter by sex
  summary(straightline_carapace_length)
```

```{r}
green_turtle %>% 
  filter(sex == "Female") %>% # filter by sex
  summary(straightline_carapace_length)
```
